...
**Versão:** 1.6 — piloto fechado (beta)
**Data:** 11 setembro 2026 (última atualização)
...
**Nota de revisão (v1.6, 11 set. 2026):** decisão registada sobre o aviso de segurança do Supabase (`leaked password protection` desativado no Auth, nível WARN nos advisors, secção 7.1). Confirmado diretamente no dashboard: a funcionalidade está indisponível no plano gratuito, só a partir do plano Pro. Mesmo raciocínio já aplicado à recusa do Softr Professional (secção 7.2): não vale gastar num upgrade de plano antes de validação de mercado (R1). Risco aceite, nível baixo (é só WARN, não bloqueia nada) — sem ação por agora; revisitar apenas se/quando o projeto migrar para um plano pago por outro motivo. **Decisão de fluxo de trabalho, mesma data:** a app GitHub associada às sessões do Claude Code não tem autorização de escrita neste repositório (erro 403 recorrente, confirmado em múltiplas tentativas). Em vez de depender de reautorização por um admin, a decisão é: **a Sara faz sempre o `git push`, manualmente, a partir do PowerShell local** — o Claude Code prepara commits localmente e comunica o conteúdo exato (diff ou ficheiro completo) para ela aplicar. Isto não é uma limitação temporária a resolver — é o processo aceite, até haver razão para mudar.

**Nota de revisão (v1.6, 11 set. 2026):** decisão registada sobre o aviso de segurança do Supabase (`leaked password protection` desativado no Auth, nível WARN nos advisors, secção 7.1). Confirmado diretamente no dashboard: a funcionalidade está indisponível no plano gratuito, só a partir do plano Pro. Mesmo raciocínio já aplicado à recusa do Softr Professional (secção 7.2): não vale gastar num upgrade de plano antes de validação de mercado (R1). Risco aceite, nível baixo (é só WARN, não bloqueia nada) — sem ação por agora; revisitar apenas se/quando o projeto migrar para um plano pago por outro motivo.
